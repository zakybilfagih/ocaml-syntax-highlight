...
let runTest = (name, f) => {
  case(
    ...
    () => {
      ...
      let pass = msg => assertString(msg, msg);
...
let tests = [
  (
    "first mate test case",
    FirstMateTestSuite.run(
      runTest,
      FirstMateTestSuite.ofDirectory("first-mate"),
    ),
  ),
  (
    "onivim test suite",
    FirstMateTestSuite.run(
      runTest,
      FirstMateTestSuite.ofDirectory("onivim"),
    ),
  ),
];
