...
let rec from_file = (~isDark=?, path: string) => {
  ...
  | Some(v) => v
  | None =>
    let currentDirectory = Filename.dirname(path);
    let themeLoader = p => {
      let fullPath = Filename.concat(currentDirectory, p);
      from_file(fullPath);
    };
    let theme =
      ...
      | Error(_) =>
        let file = SimpleXml.of_file(path);
        let parsed_file = Result.bind(file, XmlPlistParser.parse);
        Result.bind(parsed_file, PlistDecoder.theme(~isDark?));
      };
