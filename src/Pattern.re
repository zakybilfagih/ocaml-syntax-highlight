...
module Capture = {
  ...
  let show = ((index, capture): t) =>
    string_of_int(index) ++ ": " ++ capture;
};
...
let show = (v: t) =>
  ...
  | Match(patternMatch) =>
    let name =
      switch (patternMatch.matchName) {
      | None => "Name: None"
      | Some(v) => "Name: " ++ v
      };

    let captures = List.length(patternMatch.captures) |> string_of_int;
    ...
    ++ "\n"
    ++ " -"
    ++ RegExpFactory.show(patternMatch.matchRegex)
    ++ "\n)";
  | MatchRange(matchRange) =>
    let name =
...
module Json = {
  ...
  module Decode = {
    ...
    let capture = {
      ...
      let captureNested = field("name", string);
      one_of([
  ...
  let captures_of_yojson: Yojson.Safe.t => list(Capture.t) =
    json => {
      let f = keyValuePair => {
        ...
        let captureName = json |> Decode.decode_value(Decode.capture);
